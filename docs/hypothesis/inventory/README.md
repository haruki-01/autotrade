...
| ID | status | 一言 |
|----|--------|------|
| SH-01 日足規模のトレンド継続 | **機構は残る／採用不可** | プール t=+2.57 だが買い持ちに勝てない |
| SH-01N 近傍14本 | **42通り中41通りEV+、12本が t≥2** | 面としては堅い |
...
**次の判断（春希さん待ち）:** A=サイジング／レバの設計に進む（DD 1/7 を金額に変える） /
B=4時間足規模に落として回数を稼ぐ（コスト比は 2.5%→6.8〜8.6% に悪化） /
