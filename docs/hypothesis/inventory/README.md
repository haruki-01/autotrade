...
| ID | status | 一言 |
|----|--------|------|
| SH-01 日足規模のトレンド継続 | **3期間PASS（採用は保留）** | プロジェクト初。EVの桁が変わった |

`sh01_tp_4atr`: EV +0.731 / +0.595 / +0.537、DD 14〜19%、n=133/143/212（6銘柄合計）。
23本中20本が3期間ともプラス。買い持ちが −33.1% だった Set C で +38.0%（DD 18.9% 対 64.2%）。
1日遅らせても3期間プラス＝先読みではない。

**保留の理由:** 23本は1つの機構を共有しており実質1試行に近い。ゲート通過は1本だけで、
しかもそれは系統内でEVが最も低い設定（固定利確でDDと回数を稼いでいる）。
3期間とも同じ大相場の一部。
