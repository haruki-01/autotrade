...
> **現行の検証枠は固定ブラケット（2026-08-25、改訂）**
> リスクリワード **1:1.5** ／ 損切り **0.70%** ／ 利確 **1.05%** ／ 保有上限6時間 ／
> 入口は指値（損切りだけ成行）。決済率70%（90%は仮置きだったので譲った）。
> 必要上振れは旧枠の +16.9pt から **+7.4pt**（対照基準）へ。
> 15分足OHLCV 25条件で最大 +3.9pt（必要の53%）。黒字には未達だが、
> 多重検定を通った順張りが3本出た。次は OI/funding/basis。
...
**次は OHLCV 以外の入口。** 約定モデル（指値）と枠の再交渉（決済率70%、幅0.70%）は実施済み。
必要上振れの残りは +3.3pt。15分足の価格・出来高・時間帯では埋まらなさそう。
