...
### 合否軸の転換（2026-08-25）— 費用後EV>0 と DD が本命

春希さん確定: **残すのは費用後EV>0 と DD。捨てるのは月120%と月50回（合否として）。**

| 残す | 捨てる（合否にしない） |
|---|---|
| 費用後 EV>0（3期間の符号一致） | 月120%（到達可能性の別軸として TARGET_MODEL に残す） |
| DD ≤ 20% | 月50エントリー |
| `min_trades=100` は判定保留の床（下げない） | 12h / 利確1.50% のブラケット入口探索 |
...
指示により当時は **入口判断は1分〜15分足・月50エントリー以上**を条件に追加した。
`min_trades: 100`（測れるか＝判定保留の床）とは**別の軸**だった。
...
評価軸: **費用後EV>0 と DD**。月50・月120%は非ゲート。
...
旧ブラケット正本: [BRACKET_FRAME.md](../hypothesis/BRACKET_FRAME.md) ·
