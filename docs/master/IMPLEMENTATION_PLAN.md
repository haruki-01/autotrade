...
### 固定ブラケット枠での検証（2026-08-25）

指示により枠を固定した。**リスクリワード1:2固定・保有上限6時間・レバ3倍で
6時間以内決済率90%程度**。値幅は実測で決定し、損切り **0.35%** ／利確 **0.70%**
（決済率89.9%、中央保有4本=1時間）。正本は
[BRACKET_FRAME.md](../hypothesis/BRACKET_FRAME.md)。

この枠は2つの問題を解消した。**判定基準が理論から出る**（ドリフト無しの価格に
対する1:2の当たり率は1/3なので、勝率33%台なら情報ゼロと即断できる）。
**頻度も制約でなくなった**（保有上限6時間・中央保有1時間なので1枠で月260回以上）。

残った問題は損益分岐勝率で、**47.6%**。コスト比は損切り幅だけで決まり
サイジングでは動かない（0.150% / 0.35% = 42.9%）。実測の対照（無作為
エントリー）は30.7%なので、**+16.9pt の上振れが必要**。

15分足の入口条件25本 × 3期間 = 75通り、**合計254,809トレード**を検証した。

| | 結果 |
|---|---|
| 損益分岐（47.6%）超え | **0** |
| 費用後に黒字 | **0** |
| 最大の上振れ | 対照比 **+1.5pt**（`br_runs_fade_3`, z=+2.52） |
| 月50回以上 | 18 |

**25条件すべてで EV ≈ −コスト。** 検出力は十分（n=24,584で標準誤差0.30pt、
1ptの差を検出できる）なので、これは「まだ見つかっていない」ではなく
**「15分足のOHLCVには無い」**という否定的結果。

**次は A→C→B の順。** A と C はどちらも必要上振れを半分にする。

- **A. 約定モデル** — 入口を指値にすると +16.9pt → +11.7pt、両側メイカーで +8.3pt。
  未約定を数える必要がある（現行エンジンは成行のみ）
- **C. 枠の再交渉** — 決済率90%→70%なら損切り0.60%まで広げられ +9.6pt に下がる
- **B. OHLCV以外の入口** — OI/funding/basis/taker flow。EH系で15分足では
  効かなかったが、**ブラケット固定の枠での再試験はまだ**（EH系は損切りをATRで
  動かしていたので条件が違う）
